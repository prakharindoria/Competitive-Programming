...
Bon Appetit
Explanation 1
Anna didn't eat item , but she shared the rest of the items with Brian. The total cost of the shared items is  and, split in half, the cost per person is . Because ,
...
public class Solution {
    ...
    static void bonAppetit(List<Integer> bill, int k, int b) {
        ...
        int kval=bill.get(k);
        int sum=0;
        for(int i=0;i<n;i++){sum=sum+bill.get(i);}
        int annaportion=(sum-kval)/2;
        int refund=0;
        if(annaportion>=b)
        System.out.println("Bon Appetit");
        else 
        {refund=b-annaportion;
